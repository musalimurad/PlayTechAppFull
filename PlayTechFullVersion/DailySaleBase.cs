...
namespace PlayTechFullVersion
{
    public partial class DailySaleBase : Form
    {
        ...
        private void FillDataGrid()
        {
            ...
            AddProduct_dgv.DataSource = playTechDB.OrderItems.Where(m => m.Product.ProductName.Contains(Product_cb.Text) && m.DailySaleDate.Value.DayOfYear == DateTime.Now.DayOfYear).Select(b => new 
            {
            ...
            AllPrice_lb.Text = Convert.ToString(playTechDB.OrderItems.Where(x => x.DailySaleDate.Value.DayOfYear == DateTime.Now.DayOfYear).Sum(x => x.IsRefunded == true ? x.ItemPrice - x.RefundedCount * x.Product.SalePrice : x.ItemPrice) + "Azn");
            TopPrice_lb.Text = Convert.ToString(playTechDB.OrderItems.Where(x => x.DailySaleDate.Value.DayOfYear == DateTime.Now.DayOfYear).Sum(x => (decimal)(x.IsRefunded == true ? (x.ItemPrice - (x.RefundedCount * x.Product.SalePrice)) - (x.Quantity * x.Product.Price) : x.ItemPrice - (x.Quantity * x.Product.Price))) + "Azn");
